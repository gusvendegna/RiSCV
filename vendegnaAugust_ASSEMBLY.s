...
min:	
	...
	newSmallest:          # this will run if the current index is greater than a0
	add a0, t1, zero
	LoopMin:
	blt t1, a0, newSmallest      # if current item in array is less than than the current smallest, jump
	
	lw t1, 0(a2)     # load number from array into t1
	addi a2, a2, 4   # point to next element in array
	addi a3, a3, -1  # one less element remaining in array

	bne a3, zero, LoopMin  # if num of elements remaining != 0, loop, else jump back
	jalr zero, ra, 0	# return
